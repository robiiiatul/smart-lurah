...
{
    protected $table = 'request'; // Nama tabel
    protected $primaryKey = 'id_request'; // Primary key
    protected $allowedFields = [
        'id_user', 'date', 'total', 'note'
    ...
    {
        return $this->select('request.*, user.name as name')
            ->join('user', 'user.id_user = request.id_user')
            ->orderBy('request.id_request','DESC')
            ->findAll();
    }
    
    public function countDetail(int $id_request): ?int
    {
        return $this->db->table('detail-request')->where('id_request', $id_request)->where('status','Approved')->countAllResults();
    }
